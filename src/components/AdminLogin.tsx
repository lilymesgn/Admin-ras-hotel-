...
export default function AdminLogin({ onLoginSuccess, onBackToSite }: AdminLoginProps) {
  ...
  const handlePrimarySubmit = async (e: React.FormEvent) => {
    ...
    if (isDemoAccount) {
      ...
      const isSandboxRange = window.location.hostname.includes("run.app") || window.location.hostname.includes("localhost") || window.location.hostname.includes("127.0.0.1");
      
      if (isProduction && !isSandboxRange) {
